...
### 8.2 제출 전 채워야 할 효과 지표

현재 저장소는 구현 규모와 검증 흔적은 보여 주지만, AI가 얼마나 시간을 줄였는지를 직접 증명하지 않는다. 다음 값은 추정하지 않고 AgentDesk와 Git 기록으로 다시 계산한다.

| 지표 | 산정 방법 | 상태 |
|---|---|---|
| 카드 lead time | 생성 시각부터 QA 승인까지, 대기 시간 별도 표기 | 제출 전 확인 |
| AI 실행 시간 | 세션 active 시간과 tool 대기 시간 분리 | 제출 전 확인 |
| 인간 개입 시간 | 플레이 검토, 아트 승인, 방향 수정 시간을 수기 보완 | 제출 전 확인 |
| 재작업률 | QA 반려 또는 follow-up card 수 / 완료 card 수 | 제출 전 확인 |
| 검증 자동화 효과 | 반복 수동 검증 예상 시간과 실제 CI·CLI 시간 비교 | baseline 필요 |
| 모델·API 비용 | 제공자별 사용량과 비용을 카드 단위로 집계 | 제출 전 확인 |
| 산출물 채택률 | AI 제안 중 실제 main에 통합된 변경 비율 | 정의 확정 필요 |
