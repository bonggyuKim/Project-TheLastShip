...
- 현재 씬의 회전·배치가 반영된 결합 메시를 기준으로 `root > pelvis > spine > chest > head` 중심 계층을 구성했다.
- 팔은 `upper_arm > forearm > hand`, 다리는 `thigh > shin > foot` 좌우 변형 체인을 사용한다.
- 양팔과 양다리에 2본 IK, 팔꿈치·무릎 폴 컨트롤을 배치했다. 발은 IK 컨트롤 회전을 복사한다.
- 제작 중간 메시와 트레이스는 숨김 상태로 보존하고, 런타임 대상은 `LastShift_LimeAlien_Combined`, `Eye_Pupil_Sphere`, `LastShift_LimeAlien_Rig`로 구분했다.
...
- 익스트림 포즈에서 몸통, 어깨, 팔꿈치, 골반, 무릎, 발목의 실루엣과 표면 연속성을 확인한 뒤 원본은 레스트 포즈로 저장했다.
