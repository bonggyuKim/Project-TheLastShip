...
- 입 내부 고립 삼각형 제거 후 5,769개 결합 메시 정점 전체의 웨이트 할당을 확인했다.
...
- 여섯 셸의 모든 정점에서 미할당 웨이트 0개, 체인 밖 본 영향 0개, 정규화 합 최대 오차 `1.1920928955078125e-7`을 확인했다. 정본 수치는 `last-shift-lime-alien-ragdoll-shell-validation.json`과 함께 갱신한다.
