...
- 실제 입 함몰의 안쪽 20정점 링은 정원으로, 바깥 피부가 만나는 입구 24정점
  ...
  입구 안쪽 립 루프와 얼굴 쪽 첫 3개 루프를 동심 타원 흐름으로 재배치한 뒤
  고정한 네 번째 얼굴 루프에 접선 방향으로 페어링했다. 정본의 안쪽 립 루프
  엣지 간격 편차를 약 74% 줄이고 얼굴 쪽 첫 두 루프 편차도 약 절반으로
  낮추면서 입술 볼륨과 외곽 얼굴 곡률은 보존했다.
- 완화 뒤 전역 60° 결합에 더해 손목·발목 영향부를 70°, 입 내부를 89°로
  제한 정리했다.
  UV, 심, 샤프, 버텍스 컬러, 재질 경계는 넘지 않으며 관절 정리에서 삼각형
  162개, 입 내부에서 12개를 제거해, 6,305면 중 5,077면(80.5%)을 쿼드로
  전환했다.
- `Jelly_Surface_Subdivision_L1` Catmull-Clark 1단계를 제작용 blend에
  비파괴 모디파이어로 저장했다.
- 본체 제어면 6,305개, 런타임 베이크면 23,992개, 눈 표면 전체에 스무스
...
| 런타임 L1 베이크 | 24,030 | 47,984 | 100% |

베이크 메시 24,030정점은 전부 스킨 웨이트를 가지며 리그 232본과 본체 그룹
