...
- Basis와 같은 변위를 나머지 91개 셰이프키에 더해 표정 상대 변위를
  보존했다(최대 오차 `1.50e-8`).
- 실제 입 함몰의 안쪽 20정점 링과 바깥 피부가 만나는 입구 24정점 링을
  면적 보존 원으로 균등 재배치했다. 전신 완화 직후에도 둘을 다시 정원화해
  최종 입구 가로/세로 비율은 `1.000`, 반지름 편차는 `4.2e-7`이며 바깥
  주변 3링은 감쇠 연결했다.
- 완화 뒤 전역 60° 결합에 더해 손목·발목 영향부를 70°, 입 내부를 89°로
  제한 정리했다.
  UV, 심, 샤프, 버텍스 컬러, 재질 경계는 넘지 않으며 삼각형 168개를
  제거한 관절 정리에 입 내부 삼각형 10개를 추가로 제거해, 6,307면 중
  5,075면(80.5%)을 쿼드로 전환했다.
- `Jelly_Surface_Subdivision_L1` Catmull-Clark 1단계를 제작용 blend에
  비파괴 모디파이어로 저장했다.
- 본체 제어면 6,307개, 런타임 베이크면 23,996개, 눈 표면 전체에 스무스
...
| 런타임 L1 베이크 | 24,034 | 47,992 | 100% |

베이크 메시 24,034정점은 전부 스킨 웨이트를 가지며 리그 232본과 본체 그룹
