...
- 기존 4회 `0.32/-0.34` 완화를 6회 `0.38/-0.40`으로 상향했다. 왼
  사타구니–허벅지 전이의 3링, 40정점만 움직이며 바깥 실루엣과 패치 경계는
  고정했다. 최대 이동은 `0.00614`, 국소 라플라시안 거칠기는 26.42%,
  삼각형 최장/최단 변 평균은 `1.895 → 1.746`으로 감소했다.
...
`0.006519 → 0.005026`, 최대 에지 신장은 `1.542 → 1.513`으로 개선됐다.
...
도구는 정본에 `ADK_HipMouthGeometryRework_v2_strengthened` 마커를 기록한다.
이전 v1 또는 현재 v2 마커가 있으면 누적 완화하지 않고 원본 정본에서 다시
