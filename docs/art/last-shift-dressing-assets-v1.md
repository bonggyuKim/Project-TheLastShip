...
`prefab` 이 비어 있던 나머지 `25`개다. 전부 바닥·벽에 그려진 띠와 얼룩이라 **부피를 주지 않았다** — 두께 `0.02` 이하, 콜라이더 없음, 항목의 `size` 안에 들어간다. 한 일은 **단색 사각형을 패턴으로 바꾼 것**이지 크기를 키운 것이 아니다.

`14`개를 프리팹으로 갈았고 `11`개(`FloorBand`)는 그대로 뒀다.

### 9.1 발견 `1` — 경고 페인트와 배터리가 같은 표면이었다

`LS_Hazard (0.86, 0.62, 0.10)` 와 `LS_Battery (0.95, 0.65, 0.12)` 는 색이 가까운 정도가 아니라 **표면이 동일했다.** 씬 빌더의 `CreateMaterial` 이 색만 받고 `Glossiness`·`Metallic` 은 손대지 않아서, 배에 있는 모든 재질이 `0.5 / 0` 로 똑같다.

에어록은 `2700K` 등 아래라 노랑이 더 붙어 보인다. 그 방에서 경고 페인트와 집을 수 있는 배터리가 같은 노랑·같은 광택이면 **바닥 표시를 주우려고 손을 뻗게 된다.**

**색을 크게 바꾸지 않고 광택으로 갈랐다.** 경고색 정체성은 tech 가 정한 것이라 존중하고, 대신 페인트를 무광으로 내렸다.

| 재질 | 전 | 후 | 왜 |
|---|---|---|---|
| `LS_Hazard` | `(0.86, 0.62, 0.10)` / gloss `0.5` | `(0.80, 0.58, 0.11)` / gloss **`0.12`** | 페인트는 무광. 배터리는 반들거리는 채로 둔다 |
| `LS_Frost` | gloss `0.5` | gloss **`0.88`** | 얼음은 반사로 읽힌다. 색·발광은 톤에 맞아 유지 |
| `LS_Scorch` | `(0.09, 0.08, 0.08)` / gloss `0.5` | `(0.115, 0.10, 0.088)` / gloss **`0.05`** | 광택 있는 검정이라 선체(`0.18`)보다 어두운 **구멍**처럼 보였다. 완전 무광 + 미세 갈색기로 탄 자국이 된다 |

**이 셋은 리빌드에 안 덮인다.** 씬 빌더는 구조물 재질만 `CreateMaterial`(덮어씀)로 만들고, 드레싱 재질은 `EnsureMaterial`(있으면 그대로)로 만든다 — `LS_Frost` `LS_Scorch` `LS_Lane` `LS_Hazard` `LS_Fixture` `LS_ServerIndicator` `LS_GrowLight` `LS_Tint_*` 가 아트 소관이다.

### 9.2 발견 `2` — 우회 통로 유도띠가 제약 `4` 를 깨고 있었다

`DuctLane_Run`/`_Leg` 가 주 통로와 같은 `LS_Lane` 을 쓰는데, 이 재질은 **발광 `0.8`** 이다. 우회 통로는 §3.4 에서 바닥 `15 lx` 로 잡아 뒀는데, 그 안에서 유도띠가 스스로 `0.8` 로 빛나면 **등이 없어도 길이 보인다.** 브리프 §7 의 "쾌적하면 안 된다"가 재질 하나 때문에 무너진다.

**`LSD_DuctLane` 을 따로 만들었다** — `(0.42, 0.50, 0.58)` / 발광 `0.22`. 주 통로 `PassageLane` 은 `LS_Lane` 그대로다. 거기는 밝아도 되는 자리다.

형태도 바꿨다. 실선이 아니라 **셰브론**이다 — 방향을 강제해서 "지나가는 곳"으로 읽히게 한다. 머무는 곳으로 보이면 안 되는 통로다.

### 9.3 프리팹 `10`종 → 항목 `14`개

| 프리팹 | 붙는 항목 | 무엇이 달라졌나 |
|---|---|---|
| `LSDress_Decal_HazardPost` | `AirlockStripe_0..3` | 단색 기둥 → 노랑/검정 교대 밴드 `9`단. 기둥으로 읽힌다 |
| `LSDress_Decal_LaunchMark` | `LaunchMark_Fore/Aft` | 실선 `5m` → 파선 `8` + 끝 캡 `2`. 발진 유도선으로 읽힌다 |
| `LSDress_Decal_DuctLane_Run` | `DuctLane_Run` | 실선 `23.1m` → 셰브론 `16`쌍 |
| `LSDress_Decal_DuctLane_Leg` | `DuctLane_Leg` | 셰브론 `3`쌍 (`z` 방향) |
| `LSDress_Decal_Frost_Deck` | `Frost_Deck` | 사각형 → 중심 짙은 패치 `2` + 옅은 가장자리 `3` + 흩어진 조각 `2` |
| `LSDress_Decal_Frost_Wall` | `Frost_PortWall` | 패치 `4` + **흘러내린 자국 `3`** (녹았다 다시 언 자리) |
| `LSDress_Decal_Frost_Conduit` | `Frost_Conduit` | 상자 → 파이프를 감는 서리 링 `4` + 고드름 `2` |
| `LSDress_Decal_Scorch_Deck` | `Scorch_Deck` | 옅은 바깥 `1` + 짙은 중심 `2` + 튄 자국 `3` |
| `LSDress_Decal_Scorch_Wall` | `Scorch_PortWall` | 위로 갈수록 좁아지는 그을음 기둥. 열이 올라간 자국이다 |
| `LSDress_Decal_Scorch_Conduit` | `Scorch_Conduit` | 파이프 슬리브 `2` + 헤일로 `1` + 튄 자국 |

**서리·그을음은 `size` 를 안 바꿨다.** 다른 소품은 프리팹 실측치로 `size` 를 갈아 끼웠지만, 이 여섯은 `StateResponsive` 라 노출 원뿔(`z ≤ +1.40`) 판정이 `size` 에 걸려 있다. 경계를 넓히면 `C1` 의 판정 기준이 조용히 흔들린다 — 프리팹을 항목 치수 안에 들어가게 만드는 쪽을 택했다.

### 9.4 `FloorBand` `11`개는 프리팹으로 안 바꾼다

**색이 항목마다 다르기 때문이다.** 배치 훅은 `prefab` 이 있으면 `material` 칸을 무시한다(tech 가이드 명시). 그런데 `FloorBand` 는 구획마다 `LS_Tint_<Compartment>` 를 받아 그 방의 색을 말하는 유일한 자리다(`LastShiftDressing` 주석: "벽이 공통 중성색이라 라벨과 바닥 띠가 이 방의 색을 말하는 유일한 자리"). 프리팹으로 바꾸는 순간 `11`개가 전부 한 색이 되거나, 색만 다른 프리팹 `11`개를 만들어야 한다.

**색 위계가 형태 개선보다 중요하다고 판단했다.** 재질 판 그대로 두는 것이 맞다.

`LS_Tint_*` 재질 자체는 아트가 고칠 수 있지만(`EnsureMaterial`) **손대지 않았다.** 정본은 `LastShiftDressing.TintOf` 이고 EditMode 테스트가 구역색과의 거리를 검사한다 — 재질만 바꾸면 코드값과 두 벌이 되어 다음 사람이 어느 쪽이 맞는지 모른다. 조정이 필요하면 값으로 tech 에 넘긴다.

### 9.5 절대 실행하면 안 되는 메뉴

**`Last Shift/SP-02A/드레싱 에셋 부트스트랩 (덮어씀)`** — `Seed()` → `ReplaceAll(BuildInitialProps())` 라서 실행하면 **연결한 프리팹 참조 `79`개가 전부 초기값으로 되돌아간다.** 확인 대화 문구에 그 사실이 명확히 안 적혀 있다.

tech 가 병합 보존 방식으로 고칠 때까지 이 메뉴를 쓰지 않는다. 에셋이 실제로 지워졌을 때만, 그리고 그때도 이 문서의 §4·§9.3 매핑으로 다시 물릴 수 있다는 것을 확인하고 쓴다.

### 9.6 검증 (v1.1)

- **tech 검증기 통과** — `props=90 violations=0 result=PASS`. 데칼 `14`개를 물린 뒤에도 `C1`~`C4` 전부 통과
- **연결 `79`/`90`** — 끊긴 참조 `0`. 남은 `11`은 `FloorBand`(§9.4)
- **EditMode `230`개 전부 통과** (`failed 0`) — 재질 `3`종을 고친 뒤에도 구획색·구역색 거리 검사가 살아 있다
- **Unity `6000.4.0f1` 콜드 임포트 오류 `0`**

수동 확인 `3`건이 §8.1 에 더해진다.

7. **무광 경고색(`gloss 0.12`)이 `2700K` 에어록에서 배터리와 갈리는가** — 색을 거의 안 바꿨으므로 광택 하나로 갈라야 한다. 안 갈리면 색을 `(0.72, 0.52, 0.14)` 까지 내린다
8. **셰브론이 방향으로 읽히는가** — 웅크림 시점(`0.9m` 단면, 눈높이 약 `0.7m`)에서 `35°` 가 너무 눕거나 서 보일 수 있다
9. **서리·그을음 패치가 판 여러 장으로 보이는가** — 원시 도형이라 겹친 자국이 아니라 겹친 상자로 읽힐 위험이 있다. 그러면 조각 수를 줄이고 크기 차를 키운다
