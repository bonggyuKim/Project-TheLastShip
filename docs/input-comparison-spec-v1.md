...
#### 3.2.2 비증거 플레이성 pretest 1인칭 예외

`profile_id=PRETEST_FIRST_PERSON_V2`인 비증거 플레이성 pretest에서만 camera root-local `(0,+1.20,-1.25)`, pitch `-10°`와 authoritative HandMarker `(0,+0.980,0)` 조합을 허용한다. FOV `60°`, 해상도 `1920×1080`은 유지하며 center ray가 HandMarker와 gameplay depth `0` plane에서 만난다.

- `LeftArrow`/`RightArrow` hold로 spawn camera yaw 기준 `60°/s` 속도로 회전하며 `yawOffset`은 `-30°~+30°`로 clamp한다. 가속·관성은 적용하지 않는다.
- Yaw 입력은 `StrokeSession.Idle`에서만 허용한다. `Draw press`부터 `Commit` 또는 `Cancel`로 `Idle`에 복귀할 때까지 현재 `camera_visual_yaw`를 freeze한다.
- Visual camera 위치와 yaw만 조작한다. Player root rotation, spawn gameplay `n0/right0`, locomotion axis와 depth plane은 변경하지 않는다. authoritative HandMarker는 fixed child `(0,+0.980,0)`, identity rotation, unit scale이며 Aim ray는 camera에서 만들어 gameplay `n0`을 normal로 하는 stroke snapshot plane과 교차시킨다.
- 어깨너머 시점에서 본체와 손 식별용 visual mesh는 허용하되 물리 성분을 추가하지 않는다. 별도 visual hand proxy는 만들지 않고 단일 authoritative HandMarker의 손 표시와 reach boundary를 사용한다.
...
- `PRETEST_FIRST_PERSON_V2`에서는 visual camera `yawOffset=0°`
...
- Gate A 측정 build에서 yaw input action이 활성화되거나 trial 중 camera yaw가 baseline에서 `0.01°`를 초과하거나 `PRETEST_FIRST_PERSON_V2` 결과가 evaluation row에 유입되면 `TECH_INVALID/CAMERA_YAW_INPUT_ENABLED`로 기록하고 해당 attempt를 제외한다.
