...
namespace DoodleUp.Runtime
{
    ...
    public sealed class LastShiftRagdollTuning
    {
        ...
        /// </summary>
        /// <b>5.0 으로 올려 봤지만 되돌렸다(2026-08-18).</b> Rigify 리그에서 정지가 3.63초로
        /// 늘어진 것을 이 값으로 당겨 보려 했는데, 제동 토크가 관절을 한계 쪽으로 밀어붙여
        /// 머리 튕기기 초과가 0.98배 → 1.37배가 되고(팔꿈치 130도) 정지는 아예 안 섰다.
        /// 정지를 줄이려면 이 값이 아니라 다른 곳을 봐야 한다.
        ...
        /// <summary>
        /// 자기 몸끼리 충돌을 무조건 무시할 뼈 그래프 거리 상한. <c>1</c> 은 조인트로 직접 이어진
        /// 쌍(부모–자식)만 끈다는 뜻이다 — 그 쌍은 조인트가 붙잡고 있어 늘 붙어 있으므로 충돌을
        /// 켜 두면 서로 밀어내기만 한다.
        ///
        /// <b>예전 값은 99, 사실상 "전부 무시"였다.</b> 차렷 자세에서 이미 겹쳐 있는 위팔·골반 쌍이
        /// 폭발의 씨앗이라 통째로 껐던 것인데, 대가가 컸다 — 팔·몸통이 머리 메시를 그냥 통과해서
        /// 영상에서 스킨이 찢어진 것처럼 보였다. 지금은 겹치는 쌍을 <see cref="LastShiftRagdoll"/>
        /// 가 빌드 시점에 직접 재서 그 쌍만 끄므로, 촌수로 뭉뚱그려 끌 필요가 없다.
        /// </summary>
        public int SelfCollisionIgnoreDistance = 1;

        /// <summary>
        /// 조인트가 벌어져도 솔버가 도로 붙이지 않고 두는 거리(m)와 각도(도).
        ///
        /// <b>0.005m/5° 로 조여 봤지만 되돌렸다.</b> 화면의 판 모양 조각이 관절이 벌어져서
        /// 생긴 것이라면 여기를 조일 때 사라져야 하는데, 그대로 남았고 대신
        /// <c>A_bodycheck</c> 가 3.17초에 서던 것이 5초 안에 아예 안 서게 됐다 — 솔버가 매 스텝
        /// 자세를 도로 당기면서 속도가 안 죽는다. 조각의 원인은 관절 분리가 아니라는 뜻이라
        /// 값은 원래대로 두고, 이 주석을 남겨 같은 실험을 다시 하지 않게 한다.
        /// </summary>
        public float JointProjectionDistance = 0.05f;

        /// <summary>위와 같은 이유로 원래 값을 유지한다.</summary>
