...
namespace DoodleUp.Runtime
{
    ...
    ///
    /// 값 근거: -5.5 m/s^2. 지구의 약 5/9 라 체공이 지구의 1.34배로 남아 "떠 있다"가 읽히면서도,
    /// 무중력(0)처럼 착지가 불가능해져 걷기·조준이 무너지지는 않는다. 이동·조준·잡기
    /// 검증(SP-02~04)을 유지하려면 접지가 성립해야 한다.
    ///
    /// <b>달(-1.62) → 화성(-3.72) → -5.5 로 두 번 내렸다.</b> 사용자 플레이(2026-08-10)에서
    /// "점프 후 너무 늦게 떨어진다"가 두 번 나왔다 — 낙하 1.36초 → 0.90초 → <b>0.74초</b>.
    /// 지구는 0.55초다. 정점은 <see cref="JumpSpeed"/> 를 같이 올려 1.49m 로 고정한다 —
    /// 정점은 취향이 아니라 승강구 회수 높이(1.2m)와 천장 여유가 양쪽에서 좁혀 놓은 값이다.
    /// </summary>
    public static class LastShiftShipPhysics
    {
        /// <summary>선내 중력 가속도(y). 지구 -9.81 의 약 5/9.</summary>
