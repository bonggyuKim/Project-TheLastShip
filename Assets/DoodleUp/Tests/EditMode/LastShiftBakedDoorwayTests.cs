...
namespace DoodleUp.Tests.EditMode
{
    ...
    public sealed class LastShiftBakedDoorwayTests
    {
        ...
        {
            // 이 카드 전체가 이 한 줄이다 — 숙소 좌현 벽은 프리팹에 구워져 있고, 거기 붙인
            // 모듈은 지금까지 벽을 뚫고 들어갈 수 없었다.
            var ship = NewShip();
            var quarters = LastShiftCompartments.Of(LastShiftCompartment.Quarters);
            var wall = BakeCompartmentWall(ship, quarters);

            var spec = SternSpur(LastShiftCompartments.NextModuleIndex, (int)LastShiftCompartment.Quarters);
            Register(spec);
        ...
        public void LeavesALintelAboveTheOpening()
        {
            ...
            var ship = NewShip();
            var quarters = LastShiftCompartments.Of(LastShiftCompartment.Quarters);
            BakeCompartmentWall(ship, quarters);
        ...
        public void SkipsLockedModules()
        {
            ...
            var open = SternSpur(LastShiftCompartments.NextModuleIndex, (int)LastShiftCompartment.Quarters);
        ...
        public void TwoModulesOnTheSameWallEachGetTheirOwnOpening()
        {
            ...
            var first = SpurOnQuarters(LastShiftCompartments.NextModuleIndex, quarters.MinX + 1f);
            Register(first);
            var second = SpurOnQuarters(LastShiftCompartments.NextModuleIndex, quarters.MaxX - 1f);
            Register(second);
        ...
        /// 자리이고, 이 자리를 고른 이유는 고정 표에 남은 숙소와 x 대역이 안 겹치기 때문이다.
        /// </summary>
