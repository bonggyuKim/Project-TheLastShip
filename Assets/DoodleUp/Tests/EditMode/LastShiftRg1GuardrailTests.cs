...
namespace DoodleUp.Tests.EditMode
{
    ...
    ///
    /// <code>
    ///   최악 이탈       30.61m / 8.45s / Cockpit  ->  17.03m / 5.06s / Cockpit
    ///   정적 발자국비    -                         ->  1.60
    ///   실 기밀 체적비   8.46                      ->  8.80
    ...
    public sealed class LastShiftRg1GuardrailTests
    {
        ...
        /// <summary>
        /// 구역별 실 기밀 체적비의 현재값 래칫. <b>한도가 아니라 현재값이다.</b>
        ///
        /// <b>중앙 광장 허브가 이 값을 <c>3.6 → 8.85</c> 로 올렸다</b>(§9.4). M-2 시절
        /// <c>3.55</c> 는 구역 넷이 각자 방 하나씩이던 일자 스파인의 값이고, 방사형에서는
        /// 조종석 구역이 넷(광장·조종석 방·에어록 홀·숙소)의 합집합이라 실측이
        /// <c>8.80</c>(<c>264 / 30</c>)으로 뛴다 — 이 벌어짐 자체가 §3.3 이 적어 둔 회피이고,
        /// 상한으로 걸면 확정된 배치가 성립하지 않는다.
        ///
        /// <b>래칫을 실측 바로 위에 두는 것이 이 값의 용도다.</b> 실측의 두 배 위에 떠 있는
        /// 래칫은 아무것도 안 지킨다. 구역 하나가 더 커지면 여기서 먼저 걸려야 한다.
        /// </summary>
        private const float AttachedVolumeRatioRatchet = 8.85f;
